/* j/6/hike.c
**
*/
#include "all.h"


/* internal tools
*/
  /* _lily_hike_belt_root(): convert (pac) to a list of root tools.
  */
  static u3_noun
  _lily_hike_belt_root(u3_noun pac)
  {
    if ( (u3_nul == pac) ) {
      return u3_nul;
    }
    else {
      u3_atom axis      = u3x_h(u3x_h(pac));
      u3_noun tool      = u3x_t(u3x_h(pac));
      u3_noun list_tool = _lily_hike_belt_root(u3x_t(pac));

      if ( c3y == u3r_sing(1, axis) ) {
        return u3i_cell(u3a_gain(tool),
                    list_tool);
      }
      else return list_tool;
    }
  }

  /* _lily_hike_belt_l(): factor (pac) left.
  */
  static u3_noun
  _lily_hike_belt_l(u3_noun pac)
  {
    if ( (u3_nul == pac) ) {
      return u3_nul;
    }
    else {
      u3_atom axis       = u3x_h(u3x_h(pac));
      u3_noun tool       = u3x_t(u3x_h(pac));
      u3_noun belt_l     = _lily_hike_belt_l(u3x_t(pac));

      {
        if ( (1 != axis) &&
             (c3y == u3r_sing(2, u3qc_cap(axis))) )
        {
          u3_atom axis_tap = u3qc_mas(axis);

          return u3i_cell(u3i_cell(u3a_gain(axis_tap),
                           u3a_gain(tool)),
                      belt_l);
        }
        else return belt_l;
      }
    }
  }

  /* _lily_hike_belt_r(): factor (pac) right.
  */
  static u3_noun
  _lily_hike_belt_r(u3_noun pac)
  {
    if ( (u3_nul == pac) ) {
      return u3_nul;
    }
    else {
      u3_atom axis       = u3x_h(u3x_h(pac));
      u3_noun tool       = u3x_t(u3x_h(pac));
      u3_noun belt_r     = _lily_hike_belt_r(u3x_t(pac));

      {
        if ( (1 != axis) &&
             (c3y == u3r_sing(3, u3qc_cap(axis))) )
        {
          u3_atom axis_tap = u3qc_mas(axis);

          return u3i_cell(u3i_cell(u3a_gain(axis_tap),
                           u3a_gain(tool)),
                      belt_r);
        }
        else return belt_r;
      }
    }
  }

/* functions
*/
  u3_noun
  u3qf_hike(u3_noun axe,
            u3_noun pac)
  {
    c3_assert(0);
    if ( (u3_nul == pac) ) {
      return u3i_cell(0, u3a_gain(axe));
    }
    else {
      u3_noun zet = _lily_hike_belt_root(pac);

      if ( u3_nul != zet ) {
        u3_noun fol = u3a_gain(u3x_h(zet));

        u3a_lose(zet);
        return fol;
      }
      else {
        u3_noun tum = _lily_hike_belt_l(pac);
        u3_noun gam = _lily_hike_belt_r(pac);
        u3_noun hax = u3qc_peg(axe, 2);
        u3_noun moz = u3qc_peg(axe, 3);
        u3_noun zip = u3qf_hike(hax, tum);
        u3_noun dof = u3qf_hike(moz, gam);
        u3_noun fol = u3qf_cons(zip, dof);

        u3a_lose(tum);
        u3a_lose(gam);
        u3a_lose(hax);
        u3a_lose(moz);
        u3a_lose(zip);
        u3a_lose(dof);

        return fol;
      }
    }
  }
  u3_noun
  u3wf_hike(u3_noun cor)
  {
    u3_noun axe, pac;

    if ( (c3n == u3r_mean(cor, u3x_sam_2, &axe, u3x_sam_3, &pac, 0)) ||
         (c3n == u3a_is_atom(axe)) )
    {
      return u3m_bail(c3__fail);
    } else {
      return u3qf_hike(axe, pac);
    }
  }
