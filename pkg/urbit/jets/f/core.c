...
*/
            ...
            u3_noun con)
  {
    if ( (c3__void == pac) ) {
      ...
      return u3i_trel(c3__core, u3a_gain(pac), u3a_gain(con));
    }
  }
  u3_noun
  ...
  u3qf_test(const c3_c* lab_c, u3_noun sut)
  {
    ...
    if ( c3n == u3a_is_cell(sut) ) switch ( sut ) {
    ...
    else switch ( u3x_h(sut) ) {
      default: u3m_bail(c3__fail); return;

      case c3__atom: u3x_cell(u3x_t(sut), &p_sut, &q_sut);
      {
        return;
      }
      case c3__cell: u3x_cell(u3x_t(sut), &p_sut, &q_sut);
      {
        u3qf_test(lab_c, p_sut);
        u3qf_test(lab_c, q_sut);
        return;
      }
      case c3__core: u3x_cell(u3x_t(sut), &p_sut, &q_sut);
      {
        u3qf_test(lab_c, p_sut);
        return;
      }
      case c3__face: u3x_cell(u3x_t(sut), &p_sut, &q_sut);
      {
        u3qf_test(lab_c, q_sut);
        return;
      }
      case c3__fork: p_sut = u3x_t(sut);
      {
        _fork_test(lab_c, p_sut);
        return;
      }
      case c3__hint: u3x_cell(u3x_t(sut), &p_sut, &q_sut);
      {
        u3qf_test(lab_c, q_sut);
        u3qf_test(lab_c, u3x_h(p_sut));
        return;
      }
      case c3__hold: u3x_cell(u3x_t(sut), &p_sut, &q_sut);
      {
        u3qf_test(lab_c, p_sut);
        return;
      }
