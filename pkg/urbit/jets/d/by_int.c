     ...
     || (u3_nul == b) )
  {
    ...
    if ( c3y == u3qc_mor(p_n_a, p_n_b) ) {
      if ( c3y == u3r_sing(p_n_b, p_n_a) ) {
        return u3i_trel(u3a_gain(n_b), u3qdb_int(l_a, l_b), u3qdb_int(r_a, r_b));
      }
      ...
      u3_noun new_l_a = u3i_trel(u3a_gain(n_a), u3a_gain(l_a), u3_nul);
      ...
      u3_noun new_r_a = u3i_trel(u3a_gain(n_a), u3_nul, u3a_gain(r_a));
