...
_cj_axis(u3_noun fol)
{
  ...
  while ( _(u3a_is_cell(fol)) && (11 == u3h(fol)) )
    { fol = u3t(u3t(fol)); }
  ...
  if ( !_(u3r_trel(fol, &p_fol, &q_fol, &r_fol)) ) {
    ...
    if ( !_(u3a_is_cat(q_fol)) )
      { u3l_log("axis: bad c\r\n"); return 0; }
    if ( !_(u3a_is_cell(r_fol)) || (0 != u3h(r_fol)) || (1 != u3t(r_fol)) )
      { u3l_log("axis: bad d\r\n"); return 0; }
...
_cj_je_fsck(u3_noun clu)
{
  ...
  while ( _(u3a_is_cell(q_clu)) && (11 == u3h(q_clu)) ) {
    q_clu = u3t(u3t(q_clu));
  }
...
u3j_gate_prep(u3j_site* sit_u, u3_noun cor)
{
  u3_noun loc;
  u3t_on(glu_o);
  if ( c3n == u3a_is_cell(cor) || c3n == u3a_is_cell(u3t(cor)) ) {
    u3m_bail(c3__exit);
    return;
  }
  sit_u->bas   = u3_none;
